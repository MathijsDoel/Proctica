...
// header('x-forwarded-for')[0] to get IP
app.get("/", (req, res)=> {
    if(loggedIn.length == 0){
        ...
        for(let i = 0;i<loggedIn.length;i++){
            if(header('x-forwarded-for')[0] == loggedIn[i]["IP"]){
...
app.post('/login', (req, res) => {
    ...
    for(let i = 0;i<users.length;i++){
        if(username == users[i]["username"] && password == users[i]["password"]){
            loggedIn.push({"username": username, "IP": header('x-forwarded-for')[0]})
            console.log('@'+username + ' just logged in from: ' + header('x-forwarded-for')[0])
            console.log(header('x-forwarded-for')[0])
            res.redirect("/dashboard");
            break;
        }
...
app.post("/quiz/:course/:index", (req, res)=>{
    ...
    let user;
    for(let i = 0;i < loggedIn.length;i++){
        if(loggedIn[i]["IP"] == header('x-forwarded-for')[0]){
