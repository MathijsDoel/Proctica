...
// req.header('x-forwarded-for') to get IP
app.get("/", (req, res)=> {
    if(loggedIn.length == 0){
        ...
        for(let i = 0;i<loggedIn.length;i++){
            if(req.header('x-forwarded-for') == loggedIn[i]["IP"]){
...
app.post('/login', (req, res) => {
    ...
    for(let i = 0;i<users.length;i++){
        if(username == users[i]["username"] && password == users[i]["password"]){
            loggedIn.push({"username": username, "IP": req.header('x-forwarded-for')})
            console.log('@'+username + ' just logged in from: ' + req.header('x-forwarded-for'))
...
app.post("/quiz/:course/:index", (req, res)=>{
    ...
    let user;
    for(let i = 0;i < loggedIn.length;i++){
        if(loggedIn[i]["IP"] == req.header('x-forwarded-for')){
